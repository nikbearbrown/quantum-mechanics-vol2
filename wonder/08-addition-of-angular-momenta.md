...
Here is a line that astronomers love more than any other. When a radio telescope listens to the sky, the signal it hears most often — the one that maps the spiral arms of the Milky Way and finds clouds of cold hydrogen in galaxies billions of light-years off — is a single pure note from hydrogen, the 21-centimeter line. Its wavelength is 21.106 cm, its frequency 1420.405 MHz, and both numbers are so reliably hydrogen's signature that we engraved them on the Voyager probe's golden record, betting that any thinking creature out there would read them as a ruler and a clock.

So where does this famous note come from? From a tiny shuffle inside the hydrogen atom's ground state. The electron and the proton each carry spin-½. When the two spins line up, the atom sits at a slightly higher energy than when they oppose. Let the spins flip from parallel to antiparallel and the atom sheds the difference as a 21-cm photon — a jump from a state we call the triplet down to one we call the singlet.

And now the surprise. These triplet and singlet states are not some baroque construction. The triplet is $(|\!\uparrow\downarrow\rangle + |\!\downarrow\uparrow\rangle)/\sqrt{2}$; the singlet is $(|\!\uparrow\downarrow\rangle - |\!\downarrow\uparrow\rangle)/\sqrt{2}$. Look at them side by side. The only thing separating them is one minus sign. That lone sign flips the total angular momentum from $J = 1$ to $J = 0$, flips how the state behaves when you swap the two particles, and — funneled through the hyperfine Hamiltonian — sets the energy gap to exactly the size of a 21-cm photon. A minus sign you can map the galaxy with.

This chapter is about how those states get built, what they actually mean, and why the basis you naturally start in is the wrong one for computing energies.
...
Begin with the bare minimum: two particles, each spin-½, nailed down at fixed spots so the only thing left moving is their spin. One way to describe them is the obvious one. Particle 1 is up or down; particle 2 is up or down; that gives four building blocks,

$$|\!\uparrow\uparrow\rangle, \quad |\!\uparrow\downarrow\rangle, \quad |\!\downarrow\uparrow\rangle, \quad |\!\downarrow\downarrow\rangle.$$

Each of these is a definite eigenstate of $\hat{S}_{1z}$ and of $\hat{S}_{2z}$ separately. Call this the **uncoupled basis**. It is the description a bookkeeper would invent.

But here is the catch: the physics we care about almost never asks about $\hat{S}_{1z}$ or $\hat{S}_{2z}$ on their own. The electron-proton hyperfine interaction goes as $\hat{S}_e\cdot\hat{S}_p$. Spin-orbit coupling in atoms goes as $\hat{L}\cdot\hat{S}$. Both of these are blind to rotations — they commute with the total angular momentum $\hat{J} = \hat{J}_1 + \hat{J}_2$, but not with the separate pieces. So they refuse to be diagonal in the bookkeeper's basis. They are diagonal in a different one.

That other one is the **coupled basis** $|J, M\rangle$, built from simultaneous eigenstates of $\hat{J}^2 = (\hat{J}_1 + \hat{J}_2)^2$ and $\hat{J}_z = \hat{J}_{1z} + \hat{J}_{2z}$:

$$\hat{J}^2|J, M\rangle = \hbar^2 J(J+1)|J, M\rangle, \qquad \hat{J}_z|J, M\rangle = \hbar M|J, M\rangle.$$

Look carefully at what $\hat{J}^2$ actually is. It is *not* $\hat{J}_1^2 + \hat{J}_2^2$. Square the sum: $(\hat{J}_1 + \hat{J}_2)^2 = \hat{J}_1^2 + \hat{J}_2^2 + 2\hat{J}_1\cdot\hat{J}_2$. That extra $2\hat{J}_1\cdot\hat{J}_2$ is the troublemaker — it is the thing that braids the two spins together. In the coupled basis the magnitudes $j_1$ and $j_2$ survive as good labels, because $\hat{J}_1^2$ and $\hat{J}_2^2$ commute with everything in sight. What you have to give up are the individual projections $m_1$ and $m_2$; they go fuzzy.

Two bases, one and the same four-dimensional space. The **Clebsch–Gordan (CG) coefficients** are simply the entries of the unitary matrix that translates between them:
...
When you fuse $j_1$ with $j_2$, which values of $J$ are you allowed to land on? The answer is the **triangle rule**:

$$J = |j_1 - j_2|,\; |j_1 - j_2| + 1,\; \ldots,\; j_1 + j_2,$$

stepping by one each time, and each $J$ showing up exactly once.

Before you write down a single coefficient, do a sanity check on the count:

$$\sum_{J=|j_1-j_2|}^{j_1+j_2}(2J+1) = (2j_1+1)(2j_2+1).$$

This identity always holds, and it is always the first thing to verify. For $\frac{1}{2}\otimes\frac{1}{2}$: $J = 0$ and $J = 1$, so $(1) + (3) = 4 = 2\times2$. ✓ For $1\otimes\frac{1}{2}$: $J = \frac{1}{2}$ and $J = \frac{3}{2}$, so $(2) + (4) = 6 = 3\times2$. ✓ For $1\otimes1$: $J = 0, 1, 2$, so $(1)+(3)+(5) = 9 = 3\times3$. ✓

In the language of representation theory the same statement reads $j_1\otimes j_2 = |j_1-j_2|\oplus(|j_1-j_2|+1)\oplus\cdots\oplus(j_1+j_2)$. For our two spin-½ particles: $\frac{1}{2}\otimes\frac{1}{2} = 0\oplus 1$ — the four-dimensional product splits cleanly into a one-dimensional singlet and a three-dimensional triplet.
...
There is a clean, mechanical way to grind these out, and it leans on the lowering operator $\hat{J}_- = \hat{J}_{1-} + \hat{J}_{2-}$. On a coupled state it acts as

$$\hat{J}_-|J, M\rangle = \hbar\sqrt{(J+M)(J-M+1)}\,|J, M-1\rangle,$$

and on a single spin it does the obvious thing: $\hat{J}_{i-}|\!\uparrow\rangle_i = \hbar|\!\downarrow\rangle_i$ and $\hat{J}_{i-}|\!\downarrow\rangle_i = 0$ — knock an up down, and a down has nowhere to go.

The recipe has four moves, and it works for any pair $(j_1, j_2)$:

**Step 1.** Spot the stretched state $|J_\text{max}, M_\text{max}\rangle = |j_1+j_2, j_1+j_2\rangle$. It can only be the single uncoupled state with $m_1 = j_1$ and $m_2 = j_2$ — both spins maxed out. No choice, no ambiguity, nowhere to hide.

**Step 2.** Hit it with $\hat{J}_-$ over and over to sweep out the whole $J = j_1 + j_2$ family.

**Step 3.** In each $M$-sector, dig out the state belonging to $J = j_1 + j_2 - 1$ by demanding it be orthogonal to everything you have already found in that sector.

**Step 4.** Lower that one with $\hat{J}_-$ to fill out its multiplet, then orthogonalize again for the next $J$ down, and keep going.

One sign ambiguity remains, and the **Condon–Shortley phase convention** nails it down: in each coupled state, the CG coefficient carrying the largest $m_1$ (namely $m_1 = j_1$) is taken positive. Nearly everyone adopts this, and once you fix it, every other sign falls into line.
...
**State 2: $|1, 0\rangle$.** Lower both sides of that equation with $\hat{J}_-$. The left gives $\hat{J}_-|1,1\rangle = \hbar\sqrt{2}\,|1,0\rangle$. The right gives $(\hat{J}_{1-}+\hat{J}_{2-})|\!\uparrow\uparrow\rangle = \hbar|\!\downarrow\uparrow\rangle + \hbar|\!\uparrow\downarrow\rangle$. Set them equal:

$$|1, 0\rangle = \frac{1}{\sqrt{2}}\!\left(|\!\uparrow\downarrow\rangle + |\!\downarrow\uparrow\rangle\right).$$

Notice it is unchanged when you swap the two particles — symmetric.

**State 3: $|1, -1\rangle$.** Lower $|1,0\rangle$ once more. Only $\hat{J}_{1-}|\!\uparrow\downarrow\rangle = \hbar|\!\downarrow\downarrow\rangle$ and $\hat{J}_{2-}|\!\downarrow\uparrow\rangle = \hbar|\!\downarrow\downarrow\rangle$ survive:

$$|1, -1\rangle = |\!\downarrow\downarrow\rangle.$$

**State 4: $|0, 0\rangle$.** It must carry $M = 0$, so it is some mix $a|\!\uparrow\downarrow\rangle + b|\!\downarrow\uparrow\rangle$. Demand it be orthogonal to $|1,0\rangle$ and you get $a + b = 0$, hence $b = -a$. Normalize and $a = 1/\sqrt{2}$. Condon–Shortley says the coefficient of $|\!\uparrow\downarrow\rangle$ (the one with $m_1 = +\frac{1}{2}$, the larger value) comes out positive:

$$|0, 0\rangle = \frac{1}{\sqrt{2}}\!\left(|\!\uparrow\downarrow\rangle - |\!\downarrow\uparrow\rangle\right).$$

This one flips sign when you swap the particles — antisymmetric.

Check the orthogonality: $\langle 1,0|0,0\rangle = \frac{1}{2}(1-1) = 0$. ✓

All four states laid out together:
...
The three $J=1$ states make up the **triplet**. The lone $J=0$ state is the **singlet**. A mnemonic that sticks: triplets add, singlets subtract.

Now let's confront the claim that really matters — that $|0,0\rangle$ honestly has $J=0$. Throw the full $\hat{J}^2 = \hat{J}_1^2 + \hat{J}_2^2 + 2\hat{J}_{1z}\hat{J}_{2z} + \hat{J}_{1+}\hat{J}_{2-} + \hat{J}_{1-}\hat{J}_{2+}$ at the singlet:
...
For the $M=0$ sector of $\frac{1}{2}\otimes\frac{1}{2}$, the Clebsch–Gordan table reads:
...
Think of it two ways at once. Read down a column and you get a coupled state spelled out in the uncoupled basis. Read across a row and you get an uncoupled state rewritten in coupled states. To rewrite $|\!\uparrow\downarrow\rangle$, just read its row:

$$|\!\uparrow\downarrow\rangle = \frac{1}{\sqrt{2}}|1,0\rangle + \frac{1}{\sqrt{2}}|0,0\rangle.$$

And for $|\!\downarrow\uparrow\rangle$:

$$|\!\downarrow\uparrow\rangle = \frac{1}{\sqrt{2}}|1,0\rangle - \frac{1}{\sqrt{2}}|0,0\rangle.$$

That minus sign is the whole difference between the singlet and triplet pieces — and it is the most popular place to slip. Write both entries as $+1/\sqrt{2}$ and the table stops being unitary: you would have two identical columns, which is nonsense.

Six properties of CG coefficients hold for every $(j_1, j_2)$:

The coefficient $\langle j_1,j_2;m_1,m_2|J,M\rangle$ is zero unless $M = m_1 + m_2$. That is exact — the $z$-components simply add — and it is the rule that zeroes out most of the table. It also dies unless the triangle rule is satisfied. The table is orthogonal (and, in the Condon–Shortley convention, real). The coefficient carrying the largest $m_1$ in each $|J, M_\text{max}\rangle$ column is positive. Exchange symmetry ties the $(j_1, j_2)$ table to the $(j_2, j_1)$ table by a phase. And a sign-flip symmetry means that once you know every entry with $M \geq 0$, the $M < 0$ entries come for free.

---

## Why the Coupled Basis Matters: Spin-Orbit Coupling

The spin-orbit Hamiltonian is $\hat{H}_\text{so} = \lambda\,\hat{L}\cdot\hat{S}$. Lean on the identity $\hat{J} = \hat{L} + \hat{S}$ and expand:

$$\hat{J}^2 = \hat{L}^2 + \hat{S}^2 + 2\hat{L}\cdot\hat{S} \implies \hat{L}\cdot\hat{S} = \frac{1}{2}\!\left(\hat{J}^2 - \hat{L}^2 - \hat{S}^2\right).$$

In the coupled basis $|n,\ell,J,M\rangle$, every operator on the right is diagonal — and watch what that buys you:

$$\hat{H}_\text{so}|n,\ell,J,M\rangle = \frac{\lambda\hbar^2}{2}\!\left[J(J+1) - \ell(\ell+1) - s(s+1)\right]|n,\ell,J,M\rangle.$$

That is an eigenvalue equation, full stop. The energy shift is a plain number you read off. Try the same thing in the uncoupled basis $|n,\ell,m_\ell,m_s\rangle$ and $\hat{L}\cdot\hat{S}$ sprouts off-diagonal terms, mixing states of different $m_\ell$ and $m_s$ while holding $M = m_\ell + m_s$ fixed — and now you are diagonalizing a matrix by hand. The coupled basis erases that whole chore.

Take hydrogen's $2p$ state ($\ell = 1$, $s = \frac{1}{2}$): the triangle rule serves up $J = \frac{1}{2}$ (two states) and $J = \frac{3}{2}$ (four states). The splitting is
...
So $J=\frac{3}{2}$ rides $3\lambda\hbar^2/2$ above $J=\frac{1}{2}$. This is the hydrogen $2p$ fine-structure doublet — the pair of closely spaced lines you find when you look at hydrogen's Balmer series under enough magnification. Pinning down $\lambda$ takes a relativistic calculation, true. But the shape of the splitting — the structure, the $3:1$ ratio between the spacings — comes wholly from the CG algebra of this chapter.

---

## The 21-Centimeter Line

Now circle back to the opening. Hydrogen's hyperfine Hamiltonian is $\hat{H}_\text{hf} = A\,\hat{S}_e\cdot\hat{S}_p$, where $A$ packages the magnetic interaction between electron and proton. Set $\hat{F} = \hat{S}_e + \hat{S}_p$ and lean on the same identity once more:

$$\hat{S}_e\cdot\hat{S}_p = \frac{1}{2}\!\left(\hat{F}^2 - \hat{S}_e^2 - \hat{S}_p^2\right).$$

The $F=1$ triplet has $F(F+1) = 2$, with $s_e(s_e+1) = s_p(s_p+1) = \frac{3}{4}$, giving energy $A\hbar^2(2-\frac{3}{4}-\frac{3}{4})/2 = A\hbar^2/4$. The $F=0$ singlet has $F(F+1) = 0$, giving energy $A\hbar^2(0-\frac{3}{4}-\frac{3}{4})/2 = -3A\hbar^2/4$.

So the gap is $\Delta E = A\hbar^2$. For hydrogen, $A\hbar^2 = 5.87\times10^{-6}$ eV, which is a photon at $f = 1420.405$ MHz, $\lambda = 21.1$ cm.

The fall from triplet to singlet — from $(|\!\uparrow\downarrow\rangle + |\!\downarrow\uparrow\rangle)/\sqrt{2}$ to $(|\!\uparrow\downarrow\rangle - |\!\downarrow\uparrow\rangle)/\sqrt{2}$ — is that one minus sign, made audible across the galaxy.
