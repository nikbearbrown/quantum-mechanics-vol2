...
Time evolution in quantum mechanics is generated by a single unitary operator $\hat{U}(t) = e^{-i\hat{H}t/\hbar}$. You can put all the time dependence in the states (Schrödinger picture) or all of it in the operators (Heisenberg picture). Expectation values are the same either way. The Heisenberg equation of motion is the quantum counterpart of Hamilton's classical equations, with commutators replacing Poisson brackets. Ehrenfest's theorem says quantum expectation values follow classical trajectories when the wavepacket is narrow — and exactly for the harmonic oscillator.
