...
A few months later Schrödinger derived a wave equation — a partial differential equation for $\psi(x,t)$ that looked nothing like Heisenberg's matrices. The physics community briefly debated which one was right.

Before we go on — which formulation do you think is correct? Or is there a third possibility nobody had named yet?

Schrödinger showed they were equivalent. The equivalence is more than historical curiosity. The two formulations are two *pictures* of the same physics, related by the unitary operator $\hat{U}(t) = e^{-i\hat{H}t/\hbar}$. In the Schrödinger picture, $\hat{U}(t)$ acts on states. In the Heisenberg picture, it acts on operators. Expectation values — the only observables — come out the same either way.

---

## Where the Schrödinger Equation Comes From

Here is a question before the derivation: if probability must be conserved at all times, what does that demand of the operator $\hat{U}(t)$ that evolves states? Make your prediction.
...
What happens to an energy eigenstate under time evolution? Before reading: if $|\psi(0)\rangle = |E_n\rangle$ and $\hat{H}|E_n\rangle = E_n|E_n\rangle$, what is $|\psi(t)\rangle$? Does any observable change?
...
Here is the key question: the grouping $(\hat{U}^\dagger\hat{A}\hat{U})$ applied to the fixed state $|\psi(0)\rangle$ gives the same answer as $\hat{A}$ applied to the evolved state $|\psi(t)\rangle$. Can you say in words what each picture is doing with the time-dependence — and why they must agree?
...
This looks like Newton's second law — but notice the right-hand side. Is $\langle \partial V/\partial x\rangle$ the same as $\partial V/\partial \langle x\rangle$? When does the distinction matter?

They are equal only when $\partial V/\partial x$ is linear in $x$ — the harmonic oscillator and the free particle. For a general potential:
