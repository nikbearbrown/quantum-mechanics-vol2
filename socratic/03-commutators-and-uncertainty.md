...
Here is an experiment. Prepare a million electrons in exactly the same state $|\psi\rangle$. Measure position on 500,000 of them. Measure momentum on the other 500,000. No electron is measured twice. No photon kicks anything. Compute the standard deviations $\sigma_x$ and $\sigma_p$ from the two histograms.

What do you predict for the product $\sigma_x\sigma_p$?

Before we go on — think about what the popular story says. The Heisenberg microscope: the photon used to locate the electron kicks it, scrambling its momentum. That story predicts the bound comes from the *act of measurement*. In this experiment, there is no double measurement. Position instruments and momentum instruments never touch the same electron. If the microscope story is correct, there should be no constraint.

But there is. The product $\sigma_x\sigma_p \geq \hbar/2$ holds regardless. The bound was there before any measuring device turned on. It is a property of the state $|\psi\rangle$ itself, enforced by the algebra of operators during preparation, not during measurement.

The microscope story is not merely incomplete. The causal chain runs backwards. The correct statement is not "you cannot simultaneously measure position and momentum precisely." It is: *no quantum state has $\sigma_x = 0$ and $\sigma_p = 0$ simultaneously.* Robertson proved this in 1929 in a single page, from a two-line application of the Cauchy-Schwarz inequality to the commutator $[\hat{x}, \hat{p}] = i\hbar$. That commutator is where this chapter starts.

---

## What a Commutator Measures

Before we define the commutator, here is a question. Take two operations: multiply a function by $x$, then differentiate. Or differentiate first, then multiply by $x$. Should those give the same result?

Try it on $\psi(x) = e^{-x^2}$. Multiply first: $xe^{-x^2}$, then differentiate: $e^{-x^2} - 2x^2e^{-x^2}$. Differentiate first: $-2xe^{-x^2}$, then multiply: $-2x^2e^{-x^2}$. They differ by $e^{-x^2}$. The two operations don't commute — and that discrepancy is exactly $i\hbar$ worth of physics, as we will see below.

The **commutator** of two operators captures this discrepancy:
...
What does incompatibility mean concretely? If $[\hat{A}, \hat{B}] \neq 0$, no state is simultaneously an eigenstate of both. Any state sharp in $A$ (meaning $\sigma_A = 0$, i.e., an eigenstate of $\hat{A}$) necessarily has $\sigma_B > 0$. Robertson's theorem makes this quantitative.

---

## The Canonical Commutation Relation

Can you predict what $[\hat{x}, \hat{p}]$ will turn out to be? The position operator multiplies by $x$; the momentum operator is $-i\hbar\partial_x$. They act on functions. What happens when you apply them in one order versus the other?

Derive it by applying both orderings to a test function $\psi(x)$, keeping every step of the product rule visible.
...
Here is the question Robertson answered: given any two observables $\hat{A}$ and $\hat{B}$ — not just position and momentum — what is the minimum possible product of their standard deviations in any state $|\psi\rangle$?

Before reading the theorem, make a prediction. If $[\hat{A}, \hat{B}] = 0$, can you find a state with both $\sigma_A = 0$ and $\sigma_B = 0$ simultaneously? What about when $[\hat{A}, \hat{B}] \neq 0$?
...
Now here is a subtlety to think through before reading on. Suppose $[\hat{A}, \hat{B}]$ is not a constant but an operator — it depends on the state. What happens to the Robertson bound then?
...
What does this predict for the state $|\!\uparrow\rangle$, an eigenstate of $\hat{S}_z$? Make your prediction. The eigenstate has $\sigma_{S_z} = 0$. The bound is $\tfrac{\hbar}{2}|\langle\hat{S}_y\rangle_{|\uparrow\rangle}|$. In $|\!\uparrow\rangle$, we have $\langle\hat{S}_y\rangle = 0$, so the bound is zero — consistent with $\sigma_{S_z} = 0$ on the left.
...
But here is a trap: does a zero Robertson bound mean both observables can be simultaneously sharp? Think carefully before reading on.
...
Why do the quantum numbers $\ell$ and $m$ both simultaneously label states of the hydrogen atom? The answer is not a choice of convention. It is a calculation.
