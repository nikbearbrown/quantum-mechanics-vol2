...
Before reading on: what is your best guess for the physical reason? Does it require a new force between the electrons? Does it require a new particle? Or might the answer live somewhere else entirely — in a constraint on wave functions nobody had thought to impose?
...
Before reading on — what happens to a Slater determinant if two of the single-particle orbitals are identical? Make a prediction.
...
Can you trace the mechanism? Before reading: given what we just established about the exchange integral and the antisymmetric spatial wave function, why should maximizing spin lower energy?
