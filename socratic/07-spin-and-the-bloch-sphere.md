...
In February 1922, Otto Stern and Walther Gerlach sent a beam of neutral silver atoms through an inhomogeneous magnetic field and caught them on a glass plate. Before you read on, predict what they should have seen. The field gradient exerts a force proportional to the magnetic moment component along the field axis. Classical physics says the moments can point in any direction — so what shape would the deposit take? The natural answer is a smear: a continuous band, dense in the middle, thinning at the edges, because the moments are distributed over all orientations. Stern developed the plate. He found two spots.

Not a smear. Two distinct spots, nothing in between.

So the classical prediction was not merely off by a factor — it was wrong about the kind of thing that happens. Whatever silver atoms are doing, the component of their magnetic moment along the field axis takes exactly two values. And here is the part that should bother you more: this is true for any axis you point the apparatus. Two spots, always. The experiment is unambiguous, and it sat unexplained for three years — partly because Stern and Gerlach themselves thought they were confirming the old Bohr-Sommerfeld orbital quantization. But could orbital motion be the source? Silver's valence electron is in an $s$ orbital, which has zero orbital angular momentum. The two spots came from something nobody had proposed yet: intrinsic spin of the electron, hypothesized by Uhlenbeck and Goudsmit in 1925. Stern and Gerlach got the right answer before the right question existed.

---

## Two Values Forces Two Dimensions

Here is a question the experiment hands you directly: if spin always shows exactly two outcomes along any axis, how big must the space it lives in be? Resist the urge to overbuild. The temptation is to reach for something elaborate, but the data is austere — two values, nothing more — and the space should be just large enough to support that and no larger. Whatever spin lives in, that space must support any spin-component operator with exactly two eigenvalues. The smallest complex vector space that does this is $\mathbb{C}^2$. This is not a guess; it is the minimal structure forced by "two and only two."
...
Many students arrive with a picture: the electron is a tiny ball spinning on its own axis. It is a comfortable picture, and you may be holding it right now. So let us test it the way physicists test pictures — with a number — and ask whether it survives. Assume the electron has angular momentum $\hbar/2$ and the spinning-ball picture is true. What surface speed does that demand? Take the most generous possible radius — the classical electron radius $r_e \approx 2.82\times10^{-15}$ m (real experiments bound the electron below $10^{-18}$ m, but a smaller radius makes the argument worse). For a uniform solid sphere, $I = \frac{2}{5}m_e r_e^2$, and $L = I\omega$ gives equatorial surface speed:
...
Now compare. The speed of light is $c \approx 3.0\times10^8$ m/s. The required equatorial speed is roughly $170c$. Notice what this does to the picture: it does not nudge it, it demolishes it. The picture is not wrong by a small margin; it is wrong by two orders of magnitude.

So if spin is not a spinning ball, what is it? An internal degree of freedom that transforms as a representation of $\mathrm{SU}(2)$ — the group of $2\times2$ unitary matrices with determinant 1 — under rotations. Not any motion in physical space. And here is the satisfying turn: you do not even have to postulate it. Dirac showed in 1928 that spin-½ falls out of the requirement that the wave equation be Lorentz-covariant and first-order in time. You do not put spin in by hand; it is already there when you insist on the right symmetries.
...
We claimed the two-valuedness holds along *every* axis. That is a strong claim — does the formalism actually deliver it, or did we only build it in for $z$? Pick a unit vector $\hat{n} = (\sin\theta\cos\phi,\,\sin\theta\sin\phi,\,\cos\theta)$. The spin operator along $\hat{n}$ is
...
This matrix has trace zero and determinant $-(\hbar/2)^2$, so its eigenvalues are always $\pm\hbar/2$, regardless of which direction $\hat{n}$ points. The formalism delivers exactly what the 1922 experiment found: two values, every axis. We did not put that in by hand; it followed from $\mathbb{C}^2$ and the Pauli algebra.
...
Every pure spin-½ state has this form for some $(\theta, \phi)$. Now count the freedom in such a state and ask what geometric object it describes. A normalized state has two complex components, one normalization constraint, one unobservable global phase: two real parameters. Two real parameters parametrize a sphere.
...
Now ask: given a state pointing one way and an analyzer pointing another, what is the probability of "up"? Try to guess the form before reading it. The Born rule takes a clean geometric form. If the state Bloch vector points at $(\theta_\psi, \phi_\psi)$ and the analyzer points at $(\theta_n, \phi_n)$, let $\gamma$ be the angle between them:
...
A natural worry: why the half-angle? Would $\cos^2\gamma$ not do just as well? Test it at the perpendicular. At $\gamma = \pi/2$, $\cos^2(\pi/2) = 0$ but $\cos^2(\pi/4) = 1/2$. Using $\cos^2\gamma$ instead of $\cos^2(\gamma/2)$ gives zero at the perpendicular, which is wrong. The half-angle is not cosmetic; it carries the physics.

And it carries something deeper than a probability. Consider rotating the analyzer by $2\pi$ — a full turn in physical space, which should bring everything back to where it started. Should it? The angle $\theta$ goes to $\theta + 2\pi$, and $\cos(\theta/2)$ becomes $\cos(\theta/2 + \pi) = -\cos(\theta/2)$. The spin state acquires an overall minus sign. Only after $4\pi$ rotation does the state return to itself. This is the **spinor double cover**: spin states live in $\mathrm{SU}(2)$, which wraps twice around the ordinary rotation group $\mathrm{SO}(3)$. A $2\pi$ rotation that takes every vector in space back to itself takes a spinor to its negative. If that sounds like a mathematical artifact, it is not — it was directly observed in neutron interferometry by Rauch et al. (1975) and Werner et al. (1975).

---

## Larmor Precession

Put a spin in a uniform field and ask what it does over time. A spin in a uniform field $\vec{B} = B_0\hat{z}$ has Hamiltonian
...
Watch what moves and what stays put. The polar angle stays frozen. The azimuth advances at the Larmor frequency: $\phi(t) = \omega_L t$. On the Bloch sphere, the state traces a latitude circle at constant $\theta_0$, precessing around $\hat{z}$. The expectation values:
...
The spin vector precesses in the $xy$-plane at $\omega_L$ while its $z$-component is conserved. This is Larmor precession — formally identical to classical magnetic moment precession, but be careful what you are picturing: what precesses is the probability distribution over measurement outcomes, not a physical pointer.

Is this an abstraction or something you can put a number on? The numbers are clinically real. For a proton ($\gamma_p/(2\pi) = 42.58$ MHz/T):
...
Here is a sequence worth predicting before computing. Prepare a beam of silver atoms and send it through an $\mathrm{SG}_z$ apparatus. Block the lower output. The surviving atoms are in $|\!\uparrow\rangle$ — definitely up along $\hat{z}$. Send them through an $\mathrm{SG}_x$ apparatus. The $\hat{S}_x$ eigenstates are $|\hat{x},\pm\rangle = (|\!\uparrow\rangle \pm |\!\downarrow\rangle)/\sqrt{2}$. Since $|\!\uparrow\rangle = (|\hat{x},+\rangle + |\hat{x},-\rangle)/\sqrt{2}$, the Born rule gives $P(\hat{x},\pm) = 1/2$. Block the $|\hat{x},-\rangle$ output. The surviving atoms are in $|\hat{x},+\rangle = (|\!\uparrow\rangle + |\!\downarrow\rangle)/\sqrt{2}$. Now feed them into a second $\mathrm{SG}_z$. What do you expect? They started definitely up along $\hat{z}$ — surely they remember that. Result: 50/50.

The atoms started definitely spin-up along $\hat{z}$. After an intermediate $\hat{x}$ measurement, they are 50/50 along $\hat{z}$ again. The memory is gone.

The tempting interpretation: the $\mathrm{SG}_x$ device disturbed the atoms, scrambling their $z$-component. This is wrong, and the reason it is wrong is the heart of the chapter.

Look at the algebra. The operators $\hat{S}_x$ and $\hat{S}_z$ do not commute: $[\hat{S}_x, \hat{S}_z] = -i\hbar\hat{S}_y \neq 0$. No state can be simultaneously an eigenstate of both. After the $\mathrm{SG}_x$ measurement selects $|\hat{x},+\rangle$, that state simply is not an eigenstate of $\hat{S}_z$ — it has no definite $z$-component. So there was nothing there to scramble: not a definite value that the apparatus disturbed, but no definite value at all. The 50/50 at stage three is the Born rule applied to a state that genuinely has no definite $\hat{S}_z$.

Could a more careful hidden-variable picture rescue the intuition? Suppose each atom carries pre-existing values $(\mu_x, \mu_z)$ that the apparatuses merely read off. Then selecting $\mu_z = +$ at stage one and $\mu_x = +$ at stage two leaves $\mu_z$ untouched, so stage three should return $\mu_z = +$ with probability 1. Experiment gives 50/50. Pre-existing definite values for non-commuting observables are inconsistent with the data. The algebra forces this conclusion.

---

## Worked Example — Bloch Vector and Precession

A spin is prepared at Bloch angles $\theta_0 = \pi/3$, $\phi_0 = \pi/4$. We want the Born-rule probability for $+\hbar/2$ along $\hat{z}$, and then — placing this proton in $B_0 = 1.5$ T — the behavior of its precession. The two questions test different things: the first is geometry on the sphere, the second is dynamics in a field.
...
The time-evolved state is $|\psi(t)\rangle = \cos(\pi/6)|\!\uparrow\rangle + e^{i(\pi/4 + \omega_L t)}\sin(\pi/6)|\!\downarrow\rangle$. The polar angle stays at $\theta_0 = \pi/3$; the azimuth advances at $\omega_L$. So does $P(+)$ change in time? The probability $P(+)$ along $\hat{z}$ is $|\cos(\pi/6)|^2 = 3/4$ at every instant: $\langle\hat{S}_z\rangle$ is a constant of motion.

Two limiting cases are worth holding in mind. If $\theta_0 = 0$, the state is already an eigenstate of $\hat{H}$; the phase $e^{i\omega_L t}$ multiplies a zero coefficient and produces no precession. The equatorial case $\theta_0 = \pi/2$ gives maximum precession: $\langle\hat{S}_z\rangle = 0$ always, while $\langle\hat{S}_x\rangle$ and $\langle\hat{S}_y\rangle$ oscillate at full amplitude $\hbar/2$ — the Bloch vector traces the equator at 63.87 MHz.

---

## The *g*-Factor

The Dirac equation predicts $g_e = 2$ exactly. The measured value is $g_e \approx 2.00232$. Confronted with that small discrepancy, what should you conclude — that Dirac was slightly wrong? Look closer before deciding. The deviation $a_e = (g_e - 2)/2 \approx 0.00116$ is not a failure of Dirac. It is the leading QED radiative correction from virtual photons dressing the bare electron: $a_e \approx \alpha/(2\pi)$ where $\alpha \approx 1/137$ is the fine-structure constant. Agreement between QED and experiment extends to thirteen significant figures, making it one of the most precisely tested predictions in physics.

So the discrepancy is not noise to be apologized for; it is signal. When you write $\hat{H} = \gamma B_0\hat{S}_z$, use the experimental $\gamma$, which already includes the anomalous correction. The deviation from $g = 2$ is a window into the virtual-photon structure of QED, not an error to be corrected away.
