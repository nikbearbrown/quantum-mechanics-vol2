...
Here is the calculation that matters most. Before reading on: where do you predict the most-probable position of the electron in the ground state? At $r = 0$? At $r = a_0$? Somewhere else? And do you expect the most-probable position and the mean position to agree?
...
Not every transition is allowed. Here is a question to sit with before reading the rule: a photon has angular momentum 1. If the atom emits a photon, what must happen to the electron's angular momentum?

A photon carries angular momentum 1, and angular momentum conservation during emission requires the electron's orbital angular momentum to change by $\pm 1$. The electric-dipole selection rules — derived by computing $\langle\psi_f|\hat{\vec{r}}|\psi_i\rangle$ and requiring it to be nonzero — are:
...
Three pictures to be precise about. Before reading each correction, make your own prediction of what the picture is actually showing.
