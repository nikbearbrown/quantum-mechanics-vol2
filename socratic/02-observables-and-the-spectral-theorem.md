...
The momentum operator is $\hat{p} = -i\hbar\,\partial_x$. Before reading on, answer one thing for yourself: *why is the $i$ there?* Don't reach for the textbook reason — say what you actually believe. If you came up through wave mechanics, the honest answer is probably "because it works" — it produces the right de Broglie relation, the right commutators, the right everything. That is a perfectly good empirical answer. It is also a non-answer, because it tells you the $i$ is *useful* without telling you why it is *forced*.

So press on the question. Strip the $i$ out and ask what breaks. Without it, $\hat{p}$ stops being Hermitian — and a non-Hermitian operator has imaginary eigenvalues, which would mean measuring momentum returns a complex number. Picture a meter stick reading off a complex-valued length. That is not a measurement; it is nonsense. So the $i$ was never decoration and never convenience. It is necessity.

And here is the part worth sitting with: the same principle that forces the $i$ — the bare demand that measurement outcomes be *real numbers* — turns out to force a great deal more. It forces Hermitian operators, spectral decompositions, projectors, the Born rule, the whole apparatus. This chapter's wager is that you can build all of it from that single physical requirement. Watch one demand cascade into an entire framework.

---

## Why Observables Must Be Hermitian

Begin with the most modest premise you can state, and ask how far it carries. An observable is something you can measure, so its values are real. Now make that quantitative: if $\hat{A}$ represents an observable, what must be true of its expectation value? Before reading on, predict the constraint — and predict whether it has to hold for *some* states or for *all* of them.

The expectation value
$$\langle\hat{A}\rangle = \langle\psi|\hat{A}|\psi\rangle$$
must be real. And the answer to the second question is the unforgiving one: not for some states, for every state. That "every" is doing all the work, as you'll see.

Take the complex conjugate and watch what the demand for reality does. Using the definition of the adjoint — $\langle\phi|\hat{A}\psi\rangle^* = \langle\hat{A}^\dagger\phi|\psi\rangle$ — with $|\phi\rangle = |\psi\rangle$:
...
Now the "for all" earns its keep. By the polarization identity — a standard result in linear algebra — a sesquilinear form that vanishes on the diagonal vanishes everywhere. So the condition above does not merely constrain $\hat{A}$; it pins it: $\hat{A} = \hat{A}^\dagger$. The operator is **Hermitian**, or self-adjoint.

Notice what just happened, because it is easy to miss. Nobody postulated that observables are Hermitian. It came out as a three-line consequence: measurements return real numbers; real expectation values require $\hat{A} = \hat{A}^\dagger$; therefore every observable is Hermitian. The framework was required by the physics, not bolted on beside it.

Return to the momentum operator and make the abstract claim concrete. With $\hat{p} = -i\hbar\,\partial_x$:
...
So $\hat{p}^\dagger = \hat{p}$. Now run the experiment you implicitly proposed at the start of the chapter: drop the $i$ and try $\hat{p}' = \hbar\partial_x$. The same integration gives $\langle\hat{p}'\phi|\psi\rangle = -\langle\hat{p}'\phi|\psi\rangle$ — anti-Hermitian, with purely imaginary eigenvalues. Momentum is not imaginary. The $i$ is load-bearing, exactly as the opening claimed, and now you have watched the load.

One more thing before moving on, because it trips up almost everyone. In matrix language the Hermitian condition $\hat{A} = \hat{A}^\dagger$ becomes $A_{mn} = A_{nm}^*$: the matrix equals its conjugate transpose. Diagonal entries must be real (since $A_{nn} = A_{nn}^*$). Off-diagonal entries come in conjugate pairs. So ask yourself: can a matrix with complex entries be Hermitian? The reflexive answer is no — and it is wrong. The Pauli matrix $\sigma_y = \bigl(\begin{smallmatrix}0&-i\\i&0\end{smallmatrix}\bigr)$ settles it: complex entries throughout, yet $(\sigma_y)_{12} = -i = (i)^* = (\sigma_y)_{21}^*$, so Hermitian. The diagnostic question is never "are the entries real?" — it is "does $A_{mn} = A_{nm}^*$ hold?"
...
You just established that observables are Hermitian. Now predict the payoff: if $\hat{A}$ is Hermitian, what can you say about its eigenvalues? You already know what you *want* to be true — measurement outcomes are real, and eigenvalues are the possible outcomes. The question is whether Hermiticity actually delivers it, or whether you have to demand reality separately.
...
So reality was not a separate demand after all — Hermiticity hands it to you. The trick is one you should commit to memory, because the next theorem reuses it: compute the same sandwich $\langle a|\hat{A}|a\rangle$ by acting the operator on the ket and then on the bra; Hermiticity makes the bra-side return the conjugate; equating forces the eigenvalue to equal its own conjugate.

Now interrogate the proof — where exactly did Hermiticity get used, and what happens without it? In step two. Drop Hermiticity and the bra of $\hat{A}|a\rangle = a|a\rangle$ gives $\langle a|\hat{A}^\dagger = \bar{a}^*\langle a|$, where $\bar{a}$ is the eigenvalue of $\hat{A}^\dagger$ — in general a *different* number from $a$. The two computations of the sandwich no longer match, and no conclusion follows. Want to see the failure made flesh? The matrix $\bigl(\begin{smallmatrix}0&i\\i&0\end{smallmatrix}\bigr)$ is not Hermitian (check: $(A)_{12} = i$ but $A_{21}^* = i^* = -i \neq i$) and has eigenvalues $\pm i$ — purely imaginary, exactly what the theorem permits once Hermiticity is gone.

---

## Eigenstates Are Orthogonal

Here is a claim to test against your intuition before you prove it. Two eigenstates of a Hermitian operator, belonging to different eigenvalues — are they orthogonal, or could they overlap? Most people guess orthogonal, and they are right. But guessing is cheap; the question that earns the result is *why*, and the proof reuses the exact two-ways-to-compute trick you just learned.
...
Notice the two theorems are chained, not independent. The orthogonality proof leaned on $\langle a|\hat{A} = a\langle a|$, and that step is only legal because $a = a^*$ — which is precisely what the first theorem bought you. Pull out the reality result and this proof collapses at step two.

Now poke at the one hole in the argument: what happens when $a = a'$? The factor $(a' - a)$ becomes zero, and $(0)\langle a|a'\rangle = 0$ tells you exactly nothing. So two linearly independent eigenstates sharing an eigenvalue *need not* be orthogonal. If that feels like a technicality, look at the identity matrix $\hat{I}$: every vector is an eigenvector with eigenvalue 1, and there is no reason two arbitrary vectors should be orthogonal. Degeneracy genuinely breaks orthogonality.

So what do you do about it? You manufacture orthogonality where nature didn't provide it: within the degenerate subspace, run Gram-Schmidt and build an orthonormal basis by hand. The spectral theorem promises this is always possible — a degenerate subspace has a complete orthonormal eigenbasis, just not a *unique* one. That non-uniqueness is a real freedom, and in practice you spend it by finding a second observable that commutes with $\hat{A}$ and tells the degenerate states apart.

One last question, the one that turns a nuisance into a principle. Is degeneracy ever an accident? It is not. Degeneracy is always the fingerprint of a symmetry — some transformation that leaves the Hamiltonian alone but stirs the degenerate states into one another. Hydrogen's energy levels are degenerate in $\ell$ and $m_\ell$ because the Hamiltonian commutes with the angular momentum operators — rotational symmetry, written in operator form. The extra quantum numbers that label the degenerate states are the eigenvalues of those symmetry generators. Hold onto this pattern; it recurs through the rest of the course.

---

## The Spectral Theorem

You now have three facts about a Hermitian operator: real eigenvalues, orthogonal eigenstates, and a complete eigenbasis. Before reading the next line, ask what you could *build* from those three together. The answer is the central structural result of the whole subject.
...
What is the decomposition really saying? Think of writing a vector in its own eigenbasis — in that basis the operator is diagonal, with the eigenvalues $a_n$ down the diagonal. The spectral decomposition is the coordinate-free version of that picture: $\hat{A}$ acts on any state by projecting it onto each eigenspace and scaling that piece by the matching eigenvalue. Nothing more.

And the degenerate case you just worried about? It costs you nothing here. For a degenerate eigenvalue $a_n$ of multiplicity $d > 1$, the projector is $\hat{P}_n = \sum_{k=1}^d|a_n,k\rangle\langle a_n,k|$ — a sum over whatever orthonormal basis you chose for the subspace. The decomposition holds either way; the projector simply has rank $d > 1$.
...
Here is a bonus the decomposition hands you almost for free — and one you'll use constantly. Once $\hat{A}$ is written this way, you can define *any function* of it:

$$f(\hat{A}) = \sum_n f(a_n)\,|a_n\rangle\langle a_n|.$$

Does that seem like a sleight of hand? Test it on the one operator function that runs all of dynamics. The time-evolution operator $\hat{U}(t) = e^{-i\hat{H}t/\hbar}$ is built exactly this way:

$$\hat{U}(t) = \sum_n e^{-iE_n t/\hbar}|E_n\rangle\langle E_n|.$$

Read what that equation says physically. Every initial state splits into energy eigenstates, and each one simply rotates at its own frequency $E_n/\hbar$; the full evolution is the superposition of those rotations. This was not assumed anywhere — it dropped out of the spectral theorem applied to $\hat{H}$, which is Hermitian. The structure you built three sections ago already contained quantum dynamics.

---

## The Born Rule as Geometry

You have probably used the Born rule as a postulate: square the amplitude, read off the probability. Here is a more demanding question. Is probability an *extra* axiom layered onto the Hilbert-space machinery, or is it already sitting inside the geometry, waiting to be named? Predict, then watch the spectral decomposition answer.
...
Look at the last expression. The probability is the squared *length* of the projection of $|\psi\rangle$ onto the eigenspace of $a_n$. So the answer to the question is: probability was never an independent postulate. It is the squared norm of a component — a geometric quantity already living in the Hilbert space, which the spectral theorem merely lets you read off.

The expectation value follows in one line:
...
Recognize the right-hand side? It is the probability-weighted average of eigenvalues — the textbook definition of expected value from ordinary probability theory. The spectral theorem makes that correspondence exact and derivable, not a loose analogy.

Now one distinction worth stating twice, because it is the single most common error in the subject. The quantity $\langle a_n|\psi\rangle = c_n$ is a complex *amplitude*. The probability is $|c_n|^2$, its squared modulus. They differ by the act of squaring the modulus, which throws away the phase $e^{i\theta_n}$. Ask yourself why that thrown-away phase ever matters — if it vanishes from every single-outcome probability, who cares? It matters because the moment two amplitudes feed the same outcome, the phase comes roaring back as an interference term. Writing "the probability is $\langle a_n|\psi\rangle$" and landing on a complex number is the error to never make: it confuses an amplitude (complex, in the Hilbert space) with a probability (real, non-negative, a frequency of outcomes).
...
The chapter has made a string of promises: eigenvalues come out real, eigenstates come out orthogonal, the spectral sum reproduces the direct expectation value. None of those should be taken on faith. So take one concrete operator and make every promise pay out, step by step.
...
Here is the trap most people fall into, so name it before you hit it: the urge to read eigenvectors straight off the matrix without solving anything. That shortcut works only for diagonal matrices. For any non-diagonal $\hat{B}$ you have no choice — substitute the eigenvalue and solve the homogeneous system.
...
So what did the diagonalization actually buy you? Everything about the observable, in one object: the possible outcomes $\lambda_\pm$, the states $|b_\pm\rangle$ that yield each outcome with certainty, and — through one inner product — the probability of each outcome from any input. Diagonalizing $\hat{B}$ is nothing but finding the basis in which the measurement becomes transparent.

And the honest limit, the kind you should always ask after before trusting a method: scale this up to a $10\times 10$ Hermitian matrix and the secular equation is a degree-10 polynomial with no general closed-form roots. In practice you diagonalize numerically. The spectral theorem still guarantees the eigenvalues exist, are real, and span the space — it just declines to hand you a formula for them. That guarantee is enough to build the physics on.

---

## What the Commutator Will Say

The spectral theorem closed the book on a *single* observable: which outcomes are possible, which states give each with certainty, how to compute every probability. So pose the obvious next question and try to predict the answer. Given *two* observables, can you measure both at once and get a definite value for each?

Your intuition may say "of course — measure one, then the other." Hold that thought, because the answer turns on something you can compute without ever measuring anything: whether the two operators commute. If $[\hat{A}, \hat{B}] = \hat{A}\hat{B} - \hat{B}\hat{A} = 0$, the operators share a common eigenbasis and simultaneous sharp values are possible. If $[\hat{A}, \hat{B}] \neq 0$, they share no basis, and simultaneous sharp values are impossible — the uncertainty principle is the quantitative statement of that impossibility, derived from the commutator algebra itself rather than from any story about measurement disturbance. That is the next chapter, and it follows straight from the spectral theorem you just built.
