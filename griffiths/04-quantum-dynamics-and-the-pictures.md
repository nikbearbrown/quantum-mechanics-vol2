...
In 1925, Werner Heisenberg sat on the island of Helgoland and decided to work only with observable quantities: the frequencies and intensities of spectral lines. He arranged these into arrays with an unfamiliar multiplication rule — row times column, not number times number. Within months, Born and Jordan recognized that Heisenberg's arrays were matrices and that the rule $xp - px = i\hbar$ was exactly what separated quantum from classical mechanics.

Months later, Schrödinger produced a wave equation — a partial differential equation for $\psi(x,t)$ bearing no resemblance to Heisenberg's matrices. The physics community briefly wondered which was right. Schrödinger showed they were equivalent.

That equivalence is the subject of this chapter. The two formulations are two *pictures* of the same physics, related by the unitary operator $\hat{U}(t) = e^{-i\hat{H}t/\hbar}$. In the Schrödinger picture, $\hat{U}(t)$ acts on states. In the Heisenberg picture, it acts on operators. Expectation values — the only observable quantities — are identical in both.
...
The state acquires a phase but otherwise does not change. Every expectation value is constant. This is a stationary state — the same result separation of variables produces in wave mechanics, now derived from operator structure.

For a general initial state $|\psi(0)\rangle = \sum_n c_n|E_n\rangle$:

$$|\psi(t)\rangle = \sum_n c_n\,e^{-iE_n t/\hbar}|E_n\rangle.$$

The recipe: expand in energy eigenstates, attach phase $e^{-iE_n t/\hbar}$ to each, reassemble. All dynamics lives in relative phases between energy eigenstates. A single eigenstate has no relative phase — that is why it is stationary. Two or more eigenstates create beat frequencies $(E_m - E_n)/\hbar$ that drive oscillations in every observable.
...
Population starts in $|\!\uparrow\rangle$, sloshes entirely to $|\!\downarrow\rangle$ at $t = \pi/\omega_0$, and returns. This is Rabi oscillation — the quantum dynamics underlying NMR. The beat between two energy eigenstates is all there is to it.

---

## The Heisenberg Picture

Both pictures must produce identical expectation values:

$$\langle\hat{A}\rangle(t) = \langle\psi(t)|\hat{A}|\psi(t)\rangle = \langle\psi(0)|\hat{U}^\dagger(t)\hat{A}\hat{U}(t)|\psi(0)\rangle.$$

The Schrödinger picture assigns all time dependence to the state: $|\psi(t)\rangle = \hat{U}(t)|\psi(0)\rangle$, operators fixed. The Heisenberg picture reverses this: freeze the state at $t = 0$, let the operators carry all time dependence.
...
The two pictures are related by a unitary transformation — a change of basis in time. All algebraic relations survive. In particular:

$$[\hat{x}_H(t), \hat{p}_H(t)] = \hat{U}^\dagger[\hat{x}, \hat{p}]\hat{U} = i\hbar.$$

The canonical commutation relation holds at every time. Changing pictures cannot alter the fundamental algebra.
...
This is the **Heisenberg equation of motion** — an *operator* equation. Compare it to Hamilton's classical equations $\dot{x} = \{x, H\}$, $\dot{p} = \{p, H\}$, where $\{f,g\}$ is the Poisson bracket. The quantum version is the classical one with the Poisson bracket replaced by the commutator divided by $i\hbar$:

$$\{f, g\}_\text{classical} \longleftrightarrow \frac{1}{i\hbar}[\hat{f}, \hat{g}]_\text{quantum}.$$

This is Dirac's quantization dictionary. One immediate consequence: if $[\hat{H}, \hat{A}] = 0$, then $\hat{A}$ is a constant of motion. Quantum Noether theorem.
...
The position operator satisfies Newton's first law exactly — operators in place of numbers. In the Schrödinger picture, the wavepacket *spreads* — $\sigma_x$ grows with time. The Heisenberg picture says nothing about spreading; it tracks the motion of expectation values. The two pictures are complementary.
...
Hamilton's equations for the classical harmonic oscillator, with operators replacing variables. Eliminate $\hat{p}_H$:
...
The operators oscillate at frequency $\omega$ — identical to the classical result. Expectation values therefore follow classical trajectories regardless of the quantum state, which only determines the initial spread. The Heisenberg picture makes this transparent: classical dynamics lives in the operators; quantum mechanics lives in the initial state.

Verify $[\hat{x}_H(t), \hat{p}_H(t)] = i\hbar$ at all $t$: expanding using $[\hat{x},\hat{p}] = i\hbar$ and $\cos^2 + \sin^2 = 1$ gives exactly $i\hbar$. Unitarity guarantees this.
...
The annihilation operator rotates clockwise in the complex plane at frequency $\omega$. Coherent states — eigenstates of $\hat{a}$ — have expectation values tracing exact classical orbits. That is why coherent states are the most classical quantum states of the oscillator.
...
This resembles Newton's second law, but with the force evaluated at the *mean position* replaced by the *mean of the force*. They coincide only when $\partial V/\partial x$ is linear in $x$ — the harmonic oscillator and the free particle. For a general potential:

$$-\left\langle\frac{\partial V}{\partial x}\right\rangle \neq -\frac{\partial V}{\partial\langle x\rangle}.$$

The difference is of order $(\Delta x)^2 V'''(\langle x\rangle)$. When the wavepacket is narrow relative to the scale on which the force varies, the approximation is good and quantum expectation values track classical trajectories. When it fails — near a caustic, in a double well, wherever the potential is sharply curved — quantum behavior departs from classical.

For the harmonic oscillator, $V^{(n)} = 0$ for all $n \geq 3$: the potential is exactly quadratic, Ehrenfest is exact, and the expectation value follows the classical orbit precisely regardless of wavepacket shape. This is one reason the harmonic oscillator is ubiquitous in quantum mechanics.

---

## Which Picture to Use

The two pictures are not in competition. The Schrödinger picture is natural when you want the wave function itself: bound-state problems, numerical simulation, anything involving $\psi(x,t)$ directly. The Heisenberg picture is natural when you want equations of motion for observables: it produces operator equations that look like classical mechanics, making the classical limit transparent and enabling clean algebraic solutions. Quantum field theory is almost exclusively Heisenberg picture — field operators at each spacetime point satisfy local dynamical equations; Schrödinger-picture wavefunctionals for fields are unwieldy.
