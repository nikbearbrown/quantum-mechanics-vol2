...
The bound was there before any measuring device turned on. It is a property of the state $|\psi\rangle$, baked in during preparation — not something the measurement creates.

The microscope story has the causal chain backwards. The correct statement is not "you cannot simultaneously measure position and momentum precisely." It is: *no quantum state has $\sigma_x = 0$ and $\sigma_p = 0$ simultaneously.* Robertson proved this in 1929 from a two-line application of the Cauchy-Schwarz inequality to the commutator $[\hat{x}, \hat{p}] = i\hbar$. That commutator is where this chapter begins.
...
Since they commute, $\hat{B}\hat{A} = \hat{A}\hat{B}$, so $\hat{A}(\hat{B}|a\rangle) = a(\hat{B}|a\rangle)$. The vector $\hat{B}|a\rangle$ is itself an eigenvector of $\hat{A}$ with the same eigenvalue $a$. If the eigenspace at $a$ is one-dimensional, then $\hat{B}|a\rangle \propto |a\rangle$ — meaning $|a\rangle$ is simultaneously an eigenvector of $\hat{B}$. For degenerate eigenvalues, $\hat{B}$ maps the degenerate subspace into itself and can be diagonalized within it. Either way, a simultaneous eigenbasis exists.

The physical payoff is direct: in a state $|a, b\rangle$ that is simultaneously an eigenstate of $\hat{A}$ and $\hat{B}$, both observables take definite values. The **complete set of commuting observables (CSCO)** is the maximal collection of mutually commuting Hermitian operators whose joint eigenvalues uniquely label every state. For the hydrogen atom without spin, $\{\hat{H}, \hat{L}^2, \hat{L}_z\}$ forms such a set. The three quantum numbers $(n, \ell, m)$ simultaneously label every energy eigenstate not by convention but because these three operators algebraically commute. Commutativity is the reason quantum numbers exist.
...
Conversely: if $[\hat{A}, \hat{B}] \neq 0$, no state can simultaneously be an eigenstate of both. Any state sharp in $A$ — meaning $\sigma_A = 0$ — necessarily has $\sigma_B > 0$. Robertson's theorem makes this quantitative.
...
This is the **canonical commutation relation**. It is a theorem, not a postulate: it follows from $\hat{x} = $ multiplication by $x$ and $\hat{p} = -i\hbar\partial_x$, which themselves follow from requiring $\hat{p}$ to generate translations with real eigenvalues. Every statement about position-momentum incompatibility is downstream of this three-line derivation.

In three dimensions: $[\hat{x}_i, \hat{p}_j] = i\hbar\delta_{ij}$, while coordinates along different axes commute. The general pattern $[\hat{x}^n, \hat{p}] = i\hbar n\hat{x}^{n-1}$ reflects Dirac's quantization rule: the classical Poisson bracket $\{f, g\}$ becomes the commutator $[\hat{f}, \hat{g}]/i\hbar$.
...
The proof has four named steps. Work through them once and the result is yours permanently.
...
**Corollary.** Set $\hat{A} = \hat{x}$, $\hat{B} = \hat{p}$. Since $[\hat{x},\hat{p}] = i\hbar$ is a constant — not an operator whose expectation value depends on the state — $|\langle[\hat{x},\hat{p}]\rangle| = \hbar$ for every state. Therefore:

$$\sigma_x\sigma_p \geq \frac{\hbar}{2}.$$

This is state-independent. It holds for every $|\psi\rangle$ without exception, because the commutator is a c-number rather than an observable that could average to zero.
...
The state most constrained in the $y$-direction is maximally uncertain in both $x$ and $z$, and is precisely the state that saturates the Robertson bound for those two observables. The saturation condition — that the anticommutator term dropped in Step 4 equals zero — is satisfied exactly at the $\hat{S}_y$ eigenstates.
...
They cancel: $[\hat{L}^2, \hat{L}_z] = 0$. These operators are compatible. This is why $\ell$ and $m$ can simultaneously label states — why the spherical harmonics $Y_\ell^m$ are eigenfunctions of both operators at once. The quantum numbers are forced by commutativity.
