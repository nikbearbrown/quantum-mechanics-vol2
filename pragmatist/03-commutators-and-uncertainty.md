...
When $[\hat{A}, \hat{B}] = 0$, the operators are **compatible**. When nonzero, they are **incompatible**.

**Compatible operators share a complete orthonormal eigenbasis.** Proof for the non-degenerate case: suppose $[\hat{A}, \hat{B}] = 0$ and $\hat{A}|a\rangle = a|a\rangle$. Apply $\hat{B}$:

$$\hat{B}(\hat{A}|a\rangle) = a\hat{B}|a\rangle.$$

Since they commute, $\hat{B}\hat{A} = \hat{A}\hat{B}$, so $\hat{A}(\hat{B}|a\rangle) = a(\hat{B}|a\rangle)$. The vector $\hat{B}|a\rangle$ is an eigenvector of $\hat{A}$ with eigenvalue $a$. In the non-degenerate case, $\hat{B}|a\rangle \propto |a\rangle$, so $|a\rangle$ is simultaneously an eigenvector of $\hat{B}$. For degenerate eigenvalues, $\hat{B}$ maps the degenerate subspace into itself and can be diagonalized within it.

The **complete set of commuting observables (CSCO)** is the maximal collection of mutually commuting Hermitian operators whose joint eigenvalues uniquely label every state. For the hydrogen atom without spin, $\{\hat{H}, \hat{L}^2, \hat{L}_z\}$ forms a CSCO. The three quantum numbers $(n, \ell, m)$ simultaneously label every energy eigenstate because these three operators commute. The quantum numbers exist because commutativity permits them.
...
If $[\hat{A}, \hat{B}] \neq 0$, no state is simultaneously an eigenstate of both. Any state with $\sigma_A = 0$ necessarily has $\sigma_B > 0$. Robertson's theorem quantifies this.

---

## The Canonical Commutation Relation

Derive $[\hat{x}, \hat{p}]$ by applying both orderings to a test function $\psi(x)$:
...
This is the **canonical commutation relation** — a theorem following from $\hat{x} = $ multiplication by $x$ and $\hat{p} = -i\hbar\partial_x$. All position-momentum incompatibility statements are downstream of this derivation.

Extensions: in three dimensions, $[\hat{x}_i, \hat{p}_j] = i\hbar\delta_{ij}$, while coordinates along different axes commute. The pattern $[\hat{x}^n, \hat{p}] = i\hbar n\hat{x}^{n-1}$ corresponds to the Dirac quantization rule: replace the classical Poisson bracket $\{f, g\}$ with $[\hat{f}, \hat{g}]/i\hbar$.
...
**Corollary.** Set $\hat{A} = \hat{x}$, $\hat{B} = \hat{p}$. Since $[\hat{x},\hat{p}] = i\hbar$ is a c-number, $|\langle[\hat{x},\hat{p}]\rangle| = \hbar$ for every state. Therefore:

$$\sigma_x\sigma_p \geq \frac{\hbar}{2}.$$

This bound is state-independent — it holds for every $|\psi\rangle$ without exception.
...
## Worked Example: Spin-½ in an $\hat{x}$ Eigenstate

**Given.** State $|\!\uparrow_x\rangle = (|\!\uparrow\rangle + |\!\downarrow\rangle)/\sqrt{2}$. Evaluate the Robertson bound for $\hat{S}_x$ and $\hat{S}_z$.

**Find.** $\sigma_{S_x}$, $\sigma_{S_z}$, the Robertson bound, and whether it is saturated.

**Solution.**

*The commutator.* Using $\hat{S}_i = (\hbar/2)\sigma_i$ and $[\sigma_x,\sigma_z] = -2i\sigma_y$:

$$[\hat{S}_x, \hat{S}_z] = \frac{\hbar^2}{4}[\sigma_x,\sigma_z] = \frac{\hbar^2}{4}(-2i\sigma_y) = -i\hbar\hat{S}_y.$$

Robertson bound: $\sigma_{S_x}\sigma_{S_z} \geq \tfrac{\hbar}{2}|\langle\hat{S}_y\rangle|$.
...
The bound is $\tfrac{\hbar}{2}\cdot 0 = 0$.
...
Product: $\sigma_{S_x}\cdot\sigma_{S_z} = 0$. Consistent with bound $\geq 0$. $\checkmark$

**Check.** For $|+y\rangle = (|\!\uparrow\rangle + i|\!\downarrow\rangle)/\sqrt{2}$: $\langle\hat{S}_y\rangle = +\hbar/2$, bound becomes $\hbar^2/4$. In $|+y\rangle$: $\langle\hat{S}_x\rangle = \langle\hat{S}_z\rangle = 0$ and $\langle\hat{S}_{x,z}^2\rangle = \hbar^2/4$, so $\sigma_{S_x} = \sigma_{S_z} = \hbar/2$ and $\sigma_{S_x}\sigma_{S_z} = \hbar^2/4$. Bound exactly saturated. The saturation condition — anticommutator term in step 4 of the proof equals zero — is satisfied exactly at the $\hat{S}_y$ eigenstates.

---

## Compatible Observables: A Quick Diagnostic

$[\hat{L}^2, \hat{L}_z]$ determines whether $\ell$ and $m$ can simultaneously label states.

$$[\hat{L}^2, \hat{L}_z] = [\hat{L}_x^2 + \hat{L}_y^2 + \hat{L}_z^2, \hat{L}_z] = [\hat{L}_x^2, \hat{L}_z] + [\hat{L}_y^2, \hat{L}_z].$$

Using $[\hat{A}^2, \hat{B}] = \hat{A}[\hat{A},\hat{B}] + [\hat{A},\hat{B}]\hat{A}$ and the angular momentum commutators $[\hat{L}_x,\hat{L}_z] = -i\hbar\hat{L}_y$ and $[\hat{L}_y,\hat{L}_z] = i\hbar\hat{L}_x$:
...
They cancel: $[\hat{L}^2, \hat{L}_z] = 0$. The operators $\hat{L}^2$ and $\hat{L}_z$ are compatible. The quantum numbers $\ell$ and $m$ simultaneously label states, and the spherical harmonics $Y_\ell^m$ are eigenfunctions of both operators, because the algebra forces it.
...
**What saturates the Robertson bound?** Saturation requires the anticommutator term $\langle\{\hat{A}',\hat{B}'\}\rangle = 0$. For Gaussian wave packets, this holds for coherent states — minimum-uncertainty states where $\sigma_x\sigma_p = \hbar/2$ exactly. Finding minimum-uncertainty states for arbitrary operator pairs is a research-level problem.
...
This reduces to Robertson when the anticommutator term vanishes. Beyond this, **entropic uncertainty relations** (Maassen and Uffink, 1988) replace standard deviation with Shannon entropy: $H(A) + H(B) \geq \log(1/c)$ where $c = \max_{i,j}|\langle a_i|b_j\rangle|^2$. Entropic bounds are tighter for discrete observables and are used in quantum cryptography.

**Is the disturbance interpretation completely wrong?** Not exactly — it is not what Robertson's theorem says. Ozawa (2003) and Busch, Lahti, and Werner (2013) derived rigorous measurement-disturbance relations that formalize a distinct set of inequalities from Robertson's preparation uncertainty. The distinction between preparation uncertainty and disturbance uncertainty is conceptually important and not yet standard in most undergraduate curricula.
