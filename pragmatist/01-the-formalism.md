...
The same quantum state can be written two ways that look unrelated.

The wave function $\psi(x)$ gives the probability amplitude for position $x$. Its Fourier transform $\tilde{\psi}(p)$ gives the probability amplitude for momentum $p$. Both carry the complete physical content of the state, yet the two graphs need not resemble each other: a function sharply peaked in $x$ can be broad and oscillatory in $p$. A Gaussian maps to a Gaussian only because of the particular algebra of that shape; a step function in $x$ maps to a sinc function in $p$.

The question of which one is "the state" has a definite answer.

The answer is: neither. $\psi(x)$ is the state expressed in the position basis. $\tilde{\psi}(p)$ is the same state expressed in the momentum basis. The state itself is an abstract vector standing above both representations, the way a vector in three-dimensional space holds fixed while its Cartesian and spherical coordinate values differ.

Wave mechanics computes everything in a single basis, usually position. The formalism in this chapter separates the physics from the basis: write the physics once, evaluate it in whatever basis is convenient.

---

## The Arena

A **Hilbert space** $\mathcal{H}$ is a complete complex inner-product space. The two operative properties are: it is complex, and it carries an inner product. Vectors add, scale by complex numbers, and admit inner products. That is the entire structure.

The complexity is essential. Any state may be multiplied by any $c \in \mathbb{C}$ and remains a valid state. The inner product is **sesquilinear**: linear in the second argument, conjugate-linear in the first:

$$\langle(a|\phi_1\rangle + b|\phi_2\rangle)|\psi\rangle = a^*\langle\phi_1|\psi\rangle + b^*\langle\phi_2|\psi\rangle.$$

It is conjugate-symmetric, not symmetric:

$$\langle\phi|\psi\rangle = \langle\psi|\phi\rangle^*.$$

Exchanging bra and ket conjugates the value. A real inner product is symmetric and the two arguments swap freely; a complex inner product does not. That conjugate is the source of most sign errors in adjoint calculations and most confusion in Hermiticity proofs. Treat it as load-bearing.

The dimension may be finite or infinite. A qubit lives in $\mathbb{C}^2$. A particle on a line lives in $L^2(\mathbb{R})$, the square-integrable complex functions. The framework covers both with no change.

---

## Kets, Bras, and What They Are

Dirac notation keeps the type distinctions explicit.

A **ket** $|\psi\rangle$ is a vector in $\mathcal{H}$ — a column vector.

A **bra** $\langle\phi|$ is a linear functional on $\mathcal{H}$, a map sending each ket to a complex number. Every ket $|\phi\rangle$ has a corresponding bra $\langle\phi|$ defined by $\langle\phi|: |\psi\rangle \mapsto \langle\phi|\psi\rangle$ — a row vector, the conjugate transpose of the column.

The bra of $c|\phi\rangle$ is $c^*\langle\phi|$; the conjugate is forced by the definition of the inner product.

The bracket $\langle\phi|\psi\rangle$ is a **scalar**, a complex number. The reversed product $|\psi\rangle\langle\phi|$ is an **operator**, the map sending $|\chi\rangle$ to $\langle\phi|\chi\rangle\,|\psi\rangle$. Inner product gives a scalar; outer product gives an operator. They are distinct object types, and an expression that confuses them is doubly wrong.

---

## Three Identities

Fix an orthonormal basis $\{|n\rangle\}$ for $\mathcal{H}$. Three identities cover most calculations.

**Completeness:**
$$\sum_n |n\rangle\langle n| = \hat{I}.$$

This is the number 1 written as an operator. In the continuous position basis it is an integral: $\int |x\rangle\langle x|\,dx = \hat{I}$.
...
The coefficient $c_n$ is the projection of the abstract vector onto the $n$-th basis direction. Changing basis changes the numbers but not the vector.

Completeness is the working tool. Insert $\hat{I}$ between any two expressions and abstract inner products become concrete sums or integrals:

$$\langle\phi|\psi\rangle = \langle\phi|\hat{I}|\psi\rangle = \sum_n \langle\phi|n\rangle\langle n|\psi\rangle = \sum_n \phi_n^* \psi_n.$$

In the position basis this is $\int\phi^*(x)\psi(x)\,dx$. The abstract inner product and the familiar wave-function overlap integral are one object.

---

## The Fourier Transform Is a Basis Change

A particle on a line has state $|\psi\rangle$, a unit vector in $L^2(\mathbb{R})$. Two useful bases are the position eigenstates $\{|x\rangle\}$ and the momentum eigenstates $\{|p\rangle\}$.

Position-space wave function: $\psi(x) = \langle x|\psi\rangle$.

Momentum-space wave function: $\tilde{\psi}(p) = \langle p|\psi\rangle$.

These are the same vector in two bases — one state, two coordinate lists.

Insert position completeness into $\tilde{\psi}(p)$:

$$\tilde{\psi}(p) = \langle p|\psi\rangle = \int\langle p|x\rangle\langle x|\psi\rangle\,dx = \int\langle p|x\rangle\,\psi(x)\,dx.$$

The remaining quantity is $\langle p|x\rangle$. The momentum eigenstate in position space satisfies $\hat{p}|p\rangle = p|p\rangle$, i.e., $-i\hbar\,\partial_x\langle x|p\rangle = p\langle x|p\rangle$, so $\langle x|p\rangle = e^{ipx/\hbar}/\sqrt{2\pi\hbar}$. Then $\langle p|x\rangle = e^{-ipx/\hbar}/\sqrt{2\pi\hbar}$, giving:

$$\tilde{\psi}(p) = \frac{1}{\sqrt{2\pi\hbar}}\int e^{-ipx/\hbar}\psi(x)\,dx.$$

This is the Fourier transform. It was not borrowed from analysis; it came out of the completeness relation plus one differential equation. A Fourier transform in quantum mechanics is a basis change.

Position and momentum representations carry identical physics. Neither is fundamental. Work in whichever makes the calculation shorter.

---

## Operators: Abstract Maps, Not Matrices

A **linear operator** $\hat{A}$ is a linear map $\mathcal{H} \to \mathcal{H}$, defined independently of any basis.

In a basis $\{|n\rangle\}$, the operator has a matrix:

$$A_{mn} = \langle m|\hat{A}|n\rangle.$$

A different basis gives a different matrix for the same operator. This distinction is the chapter's main point.

The position operator $\hat{x}$ illustrates it. In the position basis $\hat{x}$ multiplies by $x$ — diagonal. In the harmonic-oscillator energy basis the same $\hat{x}$ is tridiagonal with entries involving $\sqrt{n}$. One operator, two unrelated-looking matrices. The matrices are basis artifacts; the operator is not.
...
In matrix form, $(A^\dagger)_{mn} = A_{nm}^*$ — the conjugate transpose. An operator is **Hermitian** when $\hat{A}^\dagger = \hat{A}$, i.e., $A_{mn} = A_{nm}^*$. The example to memorize: $\sigma_y = \bigl(\begin{smallmatrix}0&-i\\i&0\end{smallmatrix}\bigr)$. Its plain transpose is $\bigl(\begin{smallmatrix}0&i\\-i&0\end{smallmatrix}\bigr) = -\sigma_y$, antisymmetric. Its conjugate transpose is $+\sigma_y$, Hermitian. The $i$ carries the difference. A matrix with complex off-diagonal entries can be Hermitian provided $A_{12} = A_{21}^*$. The rule "Hermitian means symmetric" holds only over the reals and fails in the complex case.

The adjoint reverses product order: $(\hat{A}\hat{B})^\dagger = \hat{B}^\dagger\hat{A}^\dagger$. The proof is one line from the definition, and the result is needed whenever operators multiply.

Changing basis from $\{|a_n\rangle\}$ to $\{|b_m\rangle\}$ is done by the unitary $U_{mn} = \langle a_m|b_n\rangle$, which transforms operator matrices as $O' = U^\dagger O U$. The eigenvalues of $O'$ match those of $O$. Physics is basis-independent: the matrix is the observer's description, the operator is the object.

---

## Worked Example — A State in the Energy Basis

**Given.** A particle in an infinite square well of width $L$, prepared in

$$\psi(x) = \sqrt{\frac{30}{L^5}}\,x(L-x), \qquad 0 < x < L.$$

The energy eigenstates are $\psi_n(x) = \sqrt{2/L}\sin(n\pi x/L)$ with eigenvalues $E_n = n^2 E_1$, where $E_1 = \pi^2\hbar^2/(2mL^2)$.

**Find.** $\langle\hat{H}\rangle$, computed two ways.

**Solution.**

*Energy basis route.* The expansion coefficients are $c_n = \langle E_n|\psi\rangle = \int_0^L\psi_n^*(x)\psi(x)\,dx$. Using $\int_0^L x(L-x)\sin(n\pi x/L)\,dx = 2L^3/(n^3\pi^3)\cdot[1-(-1)^n]$, which vanishes for even $n$:
...
**Check.** Normalization of the coefficients: $\sum_{n\,\text{odd}}|c_n|^2 = 960/\pi^6 \cdot \sum_{n\,\text{odd}}1/n^6$, and $\sum_{n\,\text{odd}}1/n^6 = \pi^6/960$, so $\sum|c_n|^2 = 1$. Both routes return $\langle\hat{H}\rangle = 5\hbar^2/mL^2$. The expectation value is basis-independent: the energy basis runs a weighted discrete sum, the position basis runs two derivatives and an integral, and they agree because the operator exists independently of the basis. One limitation: the energy-basis route requires the eigenstates $\psi_n(x)$ in closed form. For a potential whose eigenstates have no analytic form, that route is not executable. The formalism is exact; whether you can compute with it depends on the basis you can write down.
