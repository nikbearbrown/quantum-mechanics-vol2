...
In classical mechanics, two identical billiard balls can always be tracked individually via continuous trajectories.

In quantum mechanics this fails completely. The wave function $\psi(\vec{r}_1, \vec{r}_2)$ spreads each electron over a region of space. When regions overlap and a detector fires, there is no fact in nature — not merely no practical way to determine, but no fact — about which electron triggered it. The labels "1" and "2" in the wave function are mathematical bookkeeping, not physical observables.
...
Swap subscripts 1 and 2: $\hat{H}(1,2) = \hat{H}(2,1)$. The physics does not distinguish the two labels.

Define the exchange operator $\hat{P}_{12}\,\psi(\vec{r}_1, \vec{r}_2) = \psi(\vec{r}_2, \vec{r}_1)$. Apply it twice: $\hat{P}_{12}^2 = \mathbb{1}$. Eigenvalues $\lambda$ satisfy $\lambda^2 = 1$, giving only $\lambda = +1$ or $\lambda = -1$.

Since $\hat{H}(1,2) = \hat{H}(2,1)$, we have $[\hat{H}, \hat{P}_{12}] = 0$: exchange symmetry is conserved under time evolution.

The empirical postulate (explained by the spin-statistics theorem of relativistic quantum field theory):
...
Composite particles inherit statistics by counting constituent fermions: an even number makes a boson, an odd number makes a fermion. $^4$He has 6 constituent fermions (even) and becomes superfluid at 2.17 K. $^3$He has 5 (odd), is a fermion, and becomes superfluid only at 2.5 mK — three orders of magnitude colder, from a single parity count.
...
Swapping particles swaps rows; swapping rows flips the sign of a determinant. Antisymmetry is a property of matrix structure, enforced automatically.
...
Swapping any two particles swaps two rows, flipping the sign. The $1/\sqrt{N!}$ normalization holds when the $\phi_i$ are orthonormal — all $N!$ terms in the expansion are orthogonal.
...
Suppose two of the single-particle orbitals are identical: $\phi_i = \phi_j$ for some $i \neq j$. Two columns of the Slater determinant are identical. A determinant with two identical columns is exactly zero — not approximately zero, identically zero everywhere in configuration space.

A wave function that is identically zero cannot be normalized. Two fermions cannot occupy the same single-particle orbital.

This is the **Pauli exclusion principle**. Pauli stated it in 1925 as an empirical rule. Slater showed four years later that it is a theorem about determinants. The quantum numbers $(n, \ell, m_\ell, m_s)$ are labels specific to atomic orbitals. In a solid-state system the relevant labels are band indices and crystal momenta — different labels, same physics. The antisymmetry argument survives unchanged in both contexts.

---

## Worked Example — Building the Antisymmetric Two-Electron State

**Given.** Both electrons in the 1s spatial orbital of helium, with spin-orbitals $\phi_A = \psi_{100}\chi_\uparrow$ and $\phi_B = \psi_{100}\chi_\downarrow$.

**Find.** The Slater determinant and the result of attempting to put both electrons in the same spin-orbital.
...
Spatial part symmetric; spin part antisymmetric (singlet $|S=0, M_S=0\rangle$). Total antisymmetric. ✓
...
**Check.** Identically zero. Two electrons in the 1s orbital must have opposite spins — not by postulate, but because a determinant with two identical columns is zero.
...
The electron-electron repulsion $V_{12}$ makes this analytically insoluble. Treat it as a perturbation on the two-electron hydrogenic problem.

The total wave function must be antisymmetric. From Chapter 8, two-spin states split into singlet ($S = 0$) and triplet ($S = 1$). Pairing with spatial parts:
...
is the **direct integral** — the classical Coulomb energy between the two charge densities.
...
is the **exchange integral** — the cross-term from antisymmetrization, swapping which electron is "in 1s" and which is "in 2s." For real orbitals with a positive Coulomb kernel, the integrand of $K$ is non-negative everywhere, so $K > 0$.

Result: parahelium 1s2s has energy $E^{(0)} + J + K$; orthohelium 1s2s has $E^{(0)} + J - K$. Since $K > 0$, orthohelium lies lower by $2K \approx 0.80$ eV.

The mechanism: $S = 1$ (triplet) requires antisymmetric spatial wave function. The antisymmetric $\psi_-$ vanishes when $\vec{r}_1 = \vec{r}_2$. Electrons in $\psi_-$ stay farther apart on average. Farther apart means less Coulomb repulsion. Lower energy. No spin-dependent force appears anywhere in this chain.

---

## Hund's First Rule

In 1925 Friedrich Hund observed: for a given electron configuration in a partially filled subshell, the term with highest total spin has the lowest energy. This is Hund's first rule.

The mechanism is the exchange integral. Higher $S$ requires a more antisymmetric spatial wave function, which keeps electrons farther apart on average, which reduces Coulomb repulsion. The rule is a corollary of antisymmetry applied to multi-electron atoms.

Oxygen ($1s^2\,2s^2\,2p^4$): Hund's rule says fill each orbital with one electron (parallel spins) before pairing. The ground state has $S = 1$, two unpaired electrons, and is paramagnetic. The $S = 1$ configuration is lower in energy because its antisymmetric spatial wave function keeps the four electrons farther apart.
...
**Screening breaks hydrogenic degeneracy.** Inner electrons partially shield the nuclear charge from outer electrons. Orbitals with low $\ell$ penetrate closer to the nucleus and are less effectively screened; at fixed $n$, low-$\ell$ states lie below high-$\ell$ states. The $n^2$-fold Coulomb degeneracy of hydrogen breaks. The 2s lies below 2p in lithium; the 3d lies above 4s in potassium.

**Pauli exclusion limits occupancy.** Each spin-orbital holds exactly one electron. Each spatial orbital holds at most two — one spin-up, one spin-down — and only with opposite spins. Without this, every electron in every atom would collapse to the 1s orbital.

**Hund's rule fills partially occupied subshells.** Within a subshell, different $m_\ell$ orbitals fill with parallel spins before pairing.

The Aufbau principle: fill orbitals in order of increasing energy (lowest $n + \ell$ first, ties broken by lowest $n$), two electrons per orbital, parallel spins first within a subshell. The sequence is $1s, 2s, 2p, 3s, 3p, 4s, 3d, 4p, \ldots$ Row lengths $2, 8, 8, 18, 18, 32$ follow from $2(2\ell + 1)$ summed over the subshells in each period.

About twenty atoms violate the Madelung energy ordering. Chromium ($3d^5\,4s^1$ rather than $3d^4\,4s^2$) and copper ($3d^{10}\,4s^1$ rather than $3d^9\,4s^2$) are the standard examples: exchange-energy stabilization of a half-filled or fully-filled 3d subshell wins against the small 3d-4s energy gap.

---

## Bosons, Briefly

Bosons have symmetric wave functions. Nothing prevents any number of them from occupying the same single-particle state — Bose-Einstein condensation. Cornell and Wieman achieved the first dilute-gas BEC in $^{87}$Rb at JILA in 1995, cooling the atoms to roughly 170 nK. Nobel Prize 2001.

One misconception: bosons are not attracted to each other and fermions are not repelled by each other in the sense of forces. There is no term in the Hamiltonian encoding exchange statistics. What exists is a correlation in $|\psi(\vec{r}_1, \vec{r}_2)|^2$ from symmetrization: fermions have a Pauli node at $\vec{r}_1 = \vec{r}_2$; bosons have enhanced probability there. The single-particle marginal density $\int|\psi|^2\,d^3r_2$ is identical for bosons, fermions, and distinguishable particles in the same pair of single-particle states. A single-particle detector cannot distinguish them.

The fermion node also supports dead stars. Electrons in a white dwarf generate an outward degeneracy pressure with no thermal origin. Chandrasekhar showed in 1931 that this pressure has a relativistic limit at approximately $1.4\,M_\odot$ — above which no equilibrium exists and gravitational collapse is inevitable. Nobel Prize 1983.
