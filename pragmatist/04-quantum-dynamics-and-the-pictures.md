...
- Time evolution is generated by a unitary operator: $|\psi(t)\rangle = \hat{U}(t)|\psi(0)\rangle$, with $\hat{U}(t) = e^{-i\hat{H}t/\hbar}$.
- The Schrödinger equation $i\hbar\,d|\psi\rangle/dt = \hat{H}|\psi\rangle$ follows from differentiating $\hat{U}(t)$.
- General time evolution: expand in energy eigenstates, attach phase $e^{-iE_n t/\hbar}$, reassemble.
- Heisenberg picture: operators carry the time dependence, $\hat{A}_H(t) = \hat{U}^\dagger\hat{A}\hat{U}$.
- Heisenberg equation: $i\hbar\,d\hat{A}_H/dt = [\hat{A}_H, \hat{H}]$.
- Both pictures give identical expectation values at all times.
