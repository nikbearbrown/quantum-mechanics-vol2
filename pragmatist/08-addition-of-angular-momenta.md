...
Start with two particles, each with spin-½. The uncoupled description gives four basis states:

$$|\!\uparrow\uparrow\rangle, \quad |\!\uparrow\downarrow\rangle, \quad |\!\downarrow\uparrow\rangle, \quad |\!\downarrow\downarrow\rangle.$$

These are eigenstates of $\hat{S}_{1z}$ and $\hat{S}_{2z}$ individually. They are the **uncoupled basis**.

The physically important interactions — hyperfine $\hat{S}_e\cdot\hat{S}_p$, spin-orbit $\hat{L}\cdot\hat{S}$ — are rotationally invariant. They commute with the total angular momentum $\hat{J} = \hat{J}_1 + \hat{J}_2$ but not with the individual components. They are diagonal in a different basis.
...
Note: $\hat{J}^2 \neq \hat{J}_1^2 + \hat{J}_2^2$. It is $(\hat{J}_1 + \hat{J}_2)^2 = \hat{J}_1^2 + \hat{J}_2^2 + 2\hat{J}_1\cdot\hat{J}_2$. The cross term $2\hat{J}_1\cdot\hat{J}_2$ mixes the individual spins. In the coupled basis, $j_1$ and $j_2$ remain good quantum numbers; $m_1$ and $m_2$ individually become uncertain.
...
Allowed $J$ values when combining $j_1$ and $j_2$:

$$J = |j_1 - j_2|,\; |j_1 - j_2| + 1,\; \ldots,\; j_1 + j_2,$$

in integer steps. Each value appears exactly once. Verify the state count first:

$$\sum_{J=|j_1-j_2|}^{j_1+j_2}(2J+1) = (2j_1+1)(2j_2+1).$$

For $\frac{1}{2}\otimes\frac{1}{2}$: $J = 0$ and $J = 1$, giving $(1) + (3) = 4 = 2\times2$. ✓ For $1\otimes\frac{1}{2}$: $J = \frac{1}{2}$ and $J = \frac{3}{2}$, giving $(2) + (4) = 6 = 3\times2$. ✓ For $1\otimes1$: $J = 0, 1, 2$, giving $(1)+(3)+(5) = 9 = 3\times3$. ✓

Representation-theoretic notation: $\frac{1}{2}\otimes\frac{1}{2} = 0\oplus 1$.
...
The systematic procedure uses the lowering operator $\hat{J}_- = \hat{J}_{1-} + \hat{J}_{2-}$:

$$\hat{J}_-|J, M\rangle = \hbar\sqrt{(J+M)(J-M+1)}\,|J, M-1\rangle,$$

and on individual spin states: $\hat{J}_{i-}|\!\uparrow\rangle_i = \hbar|\!\downarrow\rangle_i$ and $\hat{J}_{i-}|\!\downarrow\rangle_i = 0$.

**Step 1.** Identify the stretched state $|J_\text{max}, M_\text{max}\rangle = |j_1+j_2, j_1+j_2\rangle$. It equals the unique uncoupled state with $m_1 = j_1$ and $m_2 = j_2$ — no choice, no ambiguity.
...
The **Condon–Shortley phase convention** fixes the remaining sign ambiguity: the CG coefficient with the largest $m_1$ value in each coupled state is taken positive. This is almost universally adopted.

---

## The $\frac{1}{2}\otimes\frac{1}{2}$ Case Explicitly

**State 1: $|1, 1\rangle$.** The only uncoupled state with $M = +1$ is $|\!\uparrow\uparrow\rangle$:

$$|1, 1\rangle = |\!\uparrow\uparrow\rangle.$$

**State 2: $|1, 0\rangle$.** Apply $\hat{J}_-$ to both sides. Left: $\hat{J}_-|1,1\rangle = \hbar\sqrt{2}\,|1,0\rangle$. Right: $(\hat{J}_{1-}+\hat{J}_{2-})|\!\uparrow\uparrow\rangle = \hbar|\!\downarrow\uparrow\rangle + \hbar|\!\uparrow\downarrow\rangle$. Equating:
...
**State 4: $|0, 0\rangle$.** Must have $M = 0$, so it is $a|\!\uparrow\downarrow\rangle + b|\!\downarrow\uparrow\rangle$. Orthogonality to $|1,0\rangle$ forces $a + b = 0$, so $b = -a$. Normalization gives $a = 1/\sqrt{2}$. Condon–Shortley: coefficient of $|\!\uparrow\downarrow\rangle$ (larger $m_1$) is positive:
...
The three $J=1$ states are the **triplet**. The single $J=0$ state is the **singlet**. Mnemonic: triplets add, singlets subtract.

Verify that $|0,0\rangle$ has $J=0$: since $|0,0\rangle$ is orthogonal to all three $|1,M\rangle$ states and $\hat{J}^2$ commutes with $\hat{J}_z$, $\hat{J}^2|0,0\rangle$ has $M=0$ and lies in the span of $|1,0\rangle$ and $|0,0\rangle$. Computing $\langle 1,0|\hat{J}^2|0,0\rangle = 2\hbar^2\langle 1,0|0,0\rangle = 0$, so $\hat{J}^2|0,0\rangle = c|0,0\rangle$. Direct calculation gives $c = 0$. $\checkmark$
...
Each column gives a coupled state expanded in the uncoupled basis. Each row converts an uncoupled state into coupled states. To convert $|\!\uparrow\downarrow\rangle$:
...
The minus sign distinguishes the singlet coefficient from the triplet coefficient. Writing both entries as $+1/\sqrt{2}$ is wrong — it would make the table non-unitary (two identical columns).

Six properties of CG coefficients for all $(j_1, j_2)$: the coefficient $\langle j_1,j_2;m_1,m_2|J,M\rangle$ vanishes unless $M = m_1 + m_2$ (always exact) and unless the triangle rule is satisfied. The table is orthogonal (entries are real in the Condon–Shortley convention). The coefficient with the largest $m_1$ in each $|J, M_\text{max}\rangle$ column is positive. Sign-flip symmetry allows generation of $M < 0$ entries from $M \geq 0$ entries.

---

## Why the Coupled Basis Matters: Spin-Orbit Coupling

The spin-orbit Hamiltonian is $\hat{H}_\text{so} = \lambda\,\hat{L}\cdot\hat{S}$. Using $\hat{J} = \hat{L} + \hat{S}$:

$$\hat{J}^2 = \hat{L}^2 + \hat{S}^2 + 2\hat{L}\cdot\hat{S} \implies \hat{L}\cdot\hat{S} = \frac{1}{2}\!\left(\hat{J}^2 - \hat{L}^2 - \hat{S}^2\right).$$

In the coupled basis $|n,\ell,J,M\rangle$, every operator on the right is diagonal:

$$\hat{H}_\text{so}|n,\ell,J,M\rangle = \frac{\lambda\hbar^2}{2}\!\left[J(J+1) - \ell(\ell+1) - s(s+1)\right]|n,\ell,J,M\rangle.$$

This is an eigenvalue equation. In the uncoupled basis $|n,\ell,m_\ell,m_s\rangle$, $\hat{L}\cdot\hat{S}$ has off-diagonal terms coupling states with different $m_\ell$ and $m_s$ at fixed $M = m_\ell + m_s$. The coupled basis eliminates that diagonalization labor completely.

For the $2p$ state of hydrogen ($\ell = 1$, $s = \frac{1}{2}$): the triangle rule gives $J = \frac{1}{2}$ and $J = \frac{3}{2}$. The splitting is
...
The $J=\frac{3}{2}$ level sits above $J=\frac{1}{2}$ by $3\lambda\hbar^2/2$ — the hydrogen $2p$ fine-structure doublet. The splitting structure comes entirely from the CG algebra.

---

## The 21-Centimeter Line

The hydrogen hyperfine Hamiltonian is $\hat{H}_\text{hf} = A\,\hat{S}_e\cdot\hat{S}_p$. Using $\hat{F} = \hat{S}_e + \hat{S}_p$:

$$\hat{S}_e\cdot\hat{S}_p = \frac{1}{2}\!\left(\hat{F}^2 - \hat{S}_e^2 - \hat{S}_p^2\right).$$

The $F=1$ triplet: $F(F+1) = 2$, $s_e(s_e+1) = s_p(s_p+1) = \frac{3}{4}$, giving energy $A\hbar^2(2-\frac{3}{4}-\frac{3}{4})/2 = A\hbar^2/4$. The $F=0$ singlet: $F(F+1) = 0$, giving energy $A\hbar^2(0-\frac{3}{4}-\frac{3}{4})/2 = -3A\hbar^2/4$.

Splitting: $\Delta E = A\hbar^2$. For hydrogen, $A\hbar^2 = 5.87\times10^{-6}$ eV, corresponding to $f = 1420.405$ MHz and $\lambda = 21.1$ cm.

The transition from $(|\!\uparrow\downarrow\rangle + |\!\downarrow\uparrow\rangle)/\sqrt{2}$ to $(|\!\uparrow\downarrow\rangle - |\!\downarrow\uparrow\rangle)/\sqrt{2}$ is the one minus sign that makes the 21-cm line.
