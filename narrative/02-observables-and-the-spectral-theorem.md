...
Ask why the momentum operator carries a factor of $i$ — why $\hat{p} = -i\hbar\,\partial_x$ rather than $\hbar\,\partial_x$ — and you will find that most answers stop at "because it works." That answer satisfied nobody more than it should have. The $i$ is not a convention or a historical accident. It is compulsory, and the compulsion flows from a single physical demand: the outcomes of measurements must be real numbers. That demand, once made precise, forces not only the form of $\hat{p}$ but the entire scaffolding of Hermitian operators, spectral decompositions, and projectors that quantum mechanics rests on.

The architects of the theory recognized this only gradually. Heisenberg, Born, and Jordan had built matrix mechanics in 1925 without a clear criterion for which matrices counted as observables. Dirac assembled the transformation theory in 1927. It was John von Neumann, in his 1932 *Mathematische Grundlagen der Quantenmechanik*, who set the whole structure on a rigorous axiomatic foundation — establishing that the observables of quantum mechanics are precisely the self-adjoint operators on a Hilbert space, with the reality of measurement outcomes as the organizing principle. This chapter reconstructs his logic, keeping the derivations short and the physical motivation in view.

---

## Why Observables Must Be Hermitian

The starting point is minimal. An observable is a quantity that can be measured, so its values must be real. If $\hat{A}$ represents an observable, then for any state $|\psi\rangle$, the expectation value

$$\langle\hat{A}\rangle = \langle\psi|\hat{A}|\psi\rangle$$

must be real — not for some specially prepared states, but universally, for every $|\psi\rangle$ in the Hilbert space.
...
The momentum operator makes this concrete. A century ago, before matrix mechanics was even formulated, physicists knew that $\hat{p}$ had to produce real momentum values. With $\hat{p} = -i\hbar\,\partial_x$:
...
When Born proposed his probability interpretation in 1926, he was making a physical claim about the meaning of $|\psi|^2$. The spectral theorem now gives that claim a geometric home: probability is not an independent postulate but the squared norm of a projection, already present in the Hilbert-space structure.
