...
The moment a second electron enters the picture, everything changes. In helium, the electron-electron repulsion term breaks the exact solvability and, with it, the $\ell$-degeneracy. The physical mechanism that kills it is **orbital penetration and screening**: low-$\ell$ orbitals reach closer to the nucleus, see more of the unshielded nuclear charge, and sit lower in energy. High-$\ell$ orbitals, pushed outward by their centrifugal barrier, are heavily screened by inner electrons and ride higher. The result — strictly ordering orbital energies as $E(ns) < E(np) < E(nd) < E(nf)$ — is the scaffolding on which the periodic table is built.

The **central-field approximation** replaces the full intractable Hamiltonian with a sum of effective single-electron problems:
