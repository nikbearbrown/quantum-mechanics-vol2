...
This thought experiment — which has been carried out in various forms — demolishes the standard pedagogical story about Heisenberg's microscope. That story says: the photon used to locate the electron kicks it, scrambling its momentum. It produces the correct inequality by accident. But the causal chain is backwards. The correct statement is not "you cannot simultaneously measure position and momentum precisely." It is: *no quantum state has $\sigma_x = 0$ and $\sigma_p = 0$ simultaneously.* Howard Percy Robertson proved this in 1929 in a single page, from a two-line application of the Cauchy-Schwarz inequality to the commutator $[\hat{x}, \hat{p}] = i\hbar$. That commutator is where this chapter starts.
...
The single most important commutator in quantum mechanics is $[\hat{x}, \hat{p}]$. Its computation traces back to the very foundations of the subject: the moment Born and Jordan recognized in 1925 that Heisenberg's multiplication rule for quantum amplitudes was matrix algebra, the non-commutativity of position and momentum became explicit. The rule $xp - px = i\hbar$ was what distinguished quantum mechanics from classical mechanics. Here is how it falls out of the operator representations.
...
Robertson's proof has four named steps that, once traced through, make the result feel inevitable.
...
For operators whose commutator is itself an operator, the right-hand side of Robertson depends on the state through $|\langle[\hat{A},\hat{B}]\rangle|$. The canonical example is spin — the very domain Heisenberg, Born, and Pauli wrestled with in 1925 before the general formalism existed.
...
The commutator $[\hat{L}^2, \hat{L}_z]$ is worth computing explicitly, because the result underpins the entire quantum number structure of the hydrogen atom — the same structure that the spectroscopists of the 1920s were cataloguing empirically when Heisenberg's matrix mechanics gave it a theoretical home.
