...
Both bases span the same four-dimensional space. The **Clebsch–Gordan (CG) coefficients** are the entries of the unitary matrix relating them. The name honors Alfred Clebsch and Paul Gordan, who studied the mathematical problem of decomposing products of representations in the 1870s, long before quantum mechanics existed. Quantum mechanics gave their algebra a physical interpretation:
...
Return to where we started. Harold Ewen and Edward Purcell detected the 21-cm line at Harvard in 1951, a detection reported independently and almost simultaneously by groups in the Netherlands and Australia. The observation confirmed a prediction van de Hulst had made in 1944 based on precisely this physics. Here is the calculation behind it.

The hydrogen hyperfine Hamiltonian is $\hat{H}_\text{hf} = A\,\hat{S}_e\cdot\hat{S}_p$, where $A$ encodes the magnetic interaction between electron and proton. Using $\hat{F} = \hat{S}_e + \hat{S}_p$ and the same identity:
